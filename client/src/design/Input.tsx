...
/** Input — текстовое поле с меткой, иконкой, подсказкой и ошибкой; multiline → textarea. */
export function Input({
  label, value, defaultValue, placeholder, icon, hint, error,
  type = "text", multiline = false, rows = 3, disabled = false,
  onChange, id, style = {}, ...rest
}: InputProps) {
  ...
  const sharedProps = {
    id: fieldId, value, defaultValue, placeholder, disabled, onChange,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, ...style }}>
      {label && (
        <label htmlFor={fieldId} style={{ font: "var(--w-medium) 13px/1.3 var(--font-sans)", color: "var(--text-body)" }}>
          {label}
        </label>
      )}
