...
/**
 * ProposalCard — the heart of the "update layer". When AI extracts facts
 * from imported chaos it proposes a change; the user accepts / defers /
 * rejects it. Shows what changes, the backing sources, system confidence,
 * and any conflict — never auto-applied.
 */
const KINDS = {
  add_person:   { icon: "user-plus",   ru: "Новый человек" },
  merge:        { icon: "git-merge",   ru: "Объединить дубли" },
  add_fact:     { icon: "calendar-plus", ru: "Добавить факт" },
  add_alt:      { icon: "git-compare", ru: "Альтернативная версия" },
  add_place:    { icon: "map-pin",     ru: "Место проживания" },
  link_photo:   { icon: "image",       ru: "Связать фото" },
  add_source:   { icon: "paperclip",   ru: "Добавить источник" },
  conflict:     { icon: "triangle-alert", ru: "Конфликт" },
};

export function ProposalCard({
  kind = "add_fact",
  title,                 // e.g. "Дата рождения → 1928"
  ...
  const [state, setState] = useState(null); // null | accepted | deferred | rejected
  const k = KINDS[kind] || KINDS.add_fact;
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "14px 16px 12px" }}>
        ...
        {detail && (
          <p style={{
            margin: "10px 0 0", paddingLeft: 46,
      ...
      {resolved ? (
        <div style={{
          ...
          <button onClick={() => setState(null)} style={{
            ...
            color: "var(--text-muted)", font: "var(--w-medium) 12px/1 var(--font-sans)", textDecoration: "underline",
          }}>
            отменить
          </button>
        </div>
        ...
        <div style={{
          ...
          <Button size="sm" variant="primary" icon="check"
            onClick={() => { setState("accepted"); onAccept && onAccept(); }}>
            Принять
          </Button>
          <Button size="sm" variant="secondary" icon="clock"
            onClick={() => { setState("deferred"); onDefer && onDefer(); }}>
            Отложить
          </Button>
          <Button size="sm" variant="ghost" icon="x"
            onClick={() => { setState("rejected"); onReject && onReject(); }}
